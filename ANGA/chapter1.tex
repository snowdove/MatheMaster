...
\begin{satz}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item $A$ hat unendlich viele Primelemente \todo{nicht verstanden}
        \item Für jedes $0 \ne x \in A$ ist $\|x\|:= |\sfrac{A}{(x)}| < \infty$ \todo{nicht verstanden}
    \end{enumerate}
\end{satz}
\begin{proof}
    (d) $0 \ne f \in \F_q[T] \implies \|f\| = | \sfrac{\F_q[T]}{(f)} | = q^{\deg f}$.
\end{proof}
\todo{nicht verstanden}
\begin{bemerkungnr}
